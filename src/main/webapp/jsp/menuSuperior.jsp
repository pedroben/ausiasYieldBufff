<ul class="nav">
    <li>
        <a href="Controller">Inicio</a>
    </li>
    <li class="dropdown">
        <a href="#" class="dropdown-toggle" data-toggle="dropdown">Listados<b class="caret"></b></a>
        <ul class="dropdown-menu">
            <li><a href="Controller?class=usuario&method=list">Usuario</a></li>
            <li><a href="Controller?class=entrada&method=list">Entrada</a></li>
            <li><a href="Controller?class=hilo&method=list">Hilo</a></li>
            <li><a href="Controller?class=alumno&method=list">Alumno</a></li>
            <li><a href="Controller?class=profesor&method=list">Profesor</a></li>
            <li><a href="Controller?class=empresa&method=list">Empresa</a></li>
            <li><a href="Controller?class=backlog&method=list">Backlog</a></li>
        </ul>
    </li>
    <li class="dropdown">
        <a href="#" class="dropdown-toggle" data-toggle="dropdown">Altas<b class="caret"></b></a>
        <ul class="dropdown-menu">
            <li><a href="Controller?class=entrada&method=new">Entrada</a></li>
            <li><a href="Controller?class=hilo&method=new">Hilo</a></li>
            <li><a href="Controller?class=alumno&method=new">Alumno</a></li>
            <li><a href="Controller?class=profesor&method=new">Profesor</a></li>
            <li><a href="Controller?class=empresa&method=new">Empresa</a></li>
            <li><a href="Controller?class=backlog&method=new">Backlog</a></li>
        </ul>
    </li>
</ul>

